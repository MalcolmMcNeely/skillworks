...
**If an executable rule exists, run it rather than reason about it.** `docs/agents/placement-checks.md` names each command that proves placement, with its folder. Run each command in its folder, from the repo root, and nothing else. Where a row names a command to run first, run that first in the same folder, unless the path it names is there in that folder. A violation a check reports is a fact, not a judgement call, and belongs at the top of the axis. Say in the report which commands you ran; if none exists, say that too, because "this repo cannot check its own boundaries" is itself the finding a reader wants.
