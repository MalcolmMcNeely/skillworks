...
Every language shares one shape: Slices first, Concerns beneath.

- A **code root** is the folder a project's code starts in: a folder that holds a project file, such
  as a `.csproj`, and a front-end package's `src`.
- A **Slice** is the folder that holds everything one job of the app needs. `slices` names them. A
  type goes in the Slice whose job it serves.
- A **Concern** groups the types that play one role inside a Slice. In a front end `concerns` names
  them.
...
2. **A Concern comes beneath, in a front end.** A folder inside a front-end Slice is named for a
   role from `concerns`, and that list is closed. Elsewhere a folder inside a Slice is free grouping,
...
5. **`Shared` has one door.** Every folder in `Shared` names a word from the glossary of the context
   ...
   pull a Slice's code down with it. What moves is what no one job owns, however many jobs happen to
   read it.
...
A Slice name and `Shared` are matched in each language's own case: `Orders` and `Shared` in C#,
`orders` and `shared` in a front end. Rule 1 turns down a folder holding the right word in the wrong
case, and turns down `Shared` in any case below the first level.
...
- A C# namespace is the project's root namespace, then the folder path.
