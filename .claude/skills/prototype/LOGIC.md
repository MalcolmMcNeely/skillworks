...
Once the prototype has answered its question, capture the answer, then capture the prototype the way the [SKILL](SKILL.md) describes. The logic-specific mapping: the validated reducer / machine / function set lifts into the real module (the decision, absorbed); the HTML shell lands in the prototype location as the primary source — and being one self-contained file, it stays trivially re-runnable there.
