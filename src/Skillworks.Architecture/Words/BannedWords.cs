...
internal static class BannedWords
{
    ...
    public static IEnumerable<Breach> Check(string root, IReadOnlyList<string> sourceFiles, WordRules rules)
    {
        var banned = rules.BannedWords.Select(word => (Word: word, Letters: Flatten(word).Letters)).ToList();
        if (banned.Count == 0)
            yield break;
        ...
        foreach (var file in sourceFiles.Where(file => !rules.Skips(file)))
        {
            ...
            foreach (var entry in banned.Where(entry => text.Holds(entry.Letters)))
                yield return new Breach(Rule, file, $"Replace `{entry.Word}` with the word `CONTEXT.md` settles on.");
        }
    }
