...
import { readWhere, sessionAddress, withWhere } from './where';

const run = '8f1c0a9e-0000-4000-8000-000000000001';

const nowhere = { exchange: null, step: null };

describe('readWhere', () => {
  it('reads the exchange and the step a link named', () => {
    expect(readWhere(new URLSearchParams('exchange=2&step=41'))).toEqual({ exchange: 2, step: '41' });
  });
  ...
  it('leaves the repository to the Filter, which is the one thing that reads it', () => {
    ...
    expect(Object.keys(readWhere(shown))).toEqual(['exchange', 'step']);
  });
});

describe('withWhere', () => {
  it('writes where the reader is, so a reload lands in the same place', () => {
    expect(readWhere(withWhere(new URLSearchParams(''), { exchange: 2, step: '41' }))).toEqual({ exchange: 2, step: '41' });
  });

  it('takes a step off the address when the reader closes it', () => {
    expect(withWhere(new URLSearchParams('step=41&exchange=2'), nowhere).toString()).toBe('');
  });

  it('keeps the parameters it was given, so opening a step never throws the span or the repository away', () => {
    const written = withWhere(new URLSearchParams('from=2026-09-14&repository=acme%2Fxi'), { exchange: null, step: '41' });
