...
public sealed class IngestControlTests
{
    ...
    {
        using var studio = new Studio(Studio.Fixture("malformed"));
    ...
    public async Task Says_nothing_has_been_read_yet_when_it_is_pointed_at_no_transcripts()
    {
        ...
        using var studio = new Studio(machine.Subfolder("no-transcripts"));
    ...
    public async Task Reads_the_other_transcripts_when_one_of_them_will_not_open()
    {
        ...
        foreach (var project in Directory.EnumerateDirectories(Studio.Fixture("malformed")))
        {
        ...
        {
            using var studio = new Studio(transcripts);
    ...
    public async Task Reads_a_transcript_that_would_not_open_once_it_opens_again()
    {
        using var machine = new TemporaryFolder();
        var transcripts = machine.Subfolder("transcripts");
        CopyInto(Path.Combine(Studio.Fixture("malformed"), "C--Projects-kappa"), transcripts);
    ...
    public async Task Picks_up_a_session_written_while_it_is_running_without_being_asked()
    {
        ...
        using var studio = new Studio(transcripts, sweepSeconds: 1);
        await studio.WaitForIngestPasses(1);

        CopyInto(Path.Combine(Studio.Fixture("malformed"), "C--Projects-mu"), transcripts);
