...
public sealed class ActivationEndpointsTests
{
    ...
    [Fact]
    public async Task Answers_with_the_firings_then_an_end()
    {
    ...
    [Fact]
    public async Task Names_the_run_a_firing_happened_in_so_a_reader_can_open_it()
    {
        using var studio = new StudioHost();
        ...
        var firing = Assert.Single(await studio.FiringsOf("?skill=tdd"));

        Assert.Equal("tdd", firing.Skill);
        Assert.Equal(Morning, firing.Session);
        Assert.Equal(Moment("2026-09-14T09:00:00.000Z"), firing.AtUtc);
        Assert.Equal("acme/xi", firing.Repository);
        Assert.Equal("user-slash", firing.Trigger);
    }

    [Fact]
    public async Task Lists_the_firings_newest_first()
    {
        using var studio = new StudioHost();
        ...
        Assert.Equal([Afternoon, Morning], (await studio.FiringsOf("?skill=tdd")).Select(firing => firing.Session));
    }

    [Fact]
    public async Task Lists_each_firing_of_a_skill_that_fired_twice_in_one_run()
    {
        using var studio = new StudioHost();
        ...
        Assert.Equal(2, (await studio.FiringsOf("?skill=tdd")).Count);
    }

    [Fact]
    public async Task Narrows_the_firings_to_the_skill_that_was_asked_for()
    {
        using var studio = new StudioHost();
        ...
        Assert.Equal(["tdd"], (await studio.FiringsOf("?skill=tdd")).Select(firing => firing.Skill));
    }

    [Fact]
    public async Task Narrows_the_firings_to_the_span_that_was_asked_for()
    {
        using var studio = new StudioHost();
        ...
        var firings = await studio.FiringsOf("?skill=tdd&from=2026-09-14&to=2026-09-14");

        Assert.Equal([Afternoon], firings.Select(firing => firing.Session));
    }

    [Fact]
    public async Task Narrows_the_firings_to_the_repository_that_was_asked_for()
    {
        using var studio = new StudioHost();
        ...
        Assert.Equal([Morning], (await studio.FiringsOf("?skill=tdd&repository=acme/xi")).Select(firing => firing.Session));
    }

    [Fact]
    public async Task Names_no_repository_for_a_firing_in_a_checkout_that_has_none()
    {
        using var studio = new StudioHost();

        await studio.Push(Fired("tdd", "2026-09-14T09:00:00.000Z", Morning));

        Assert.Null(Assert.Single(await studio.FiringsOf("?skill=tdd")).Repository);
    }

    [Fact]
    public async Task Answers_no_firings_for_a_skill_that_never_fired()
    {
        using var studio = new StudioHost();
    ...
    [Fact]
    public async Task Names_the_events_store_when_the_firings_cannot_be_read()
    {
