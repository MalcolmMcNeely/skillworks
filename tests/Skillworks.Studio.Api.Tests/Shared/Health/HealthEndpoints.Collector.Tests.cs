...
public sealed partial class HealthEndpointsTests
{
    ...
    public async Task Reports_a_collector_that_refuses_spans_as_broken_and_names_the_shut_door()
    {
        ...
        using var studio = new StudioHost(StudioHost.Marketplace(), collector: collector);
    ...
    public async Task Never_reads_the_collector_as_off_whether_it_is_answering_or_not()
    {
        ...
        using var stopped = new StudioHost(StudioHost.Marketplace(), collector: down, emitting: false, tracing: false);
        using var running = new StudioHost(StudioHost.Marketplace(), emitting: false, tracing: false);
    ...
    {
        using var collector = FakeCollector.SpansShut();
        using var shut = new StudioHost(StudioHost.Marketplace(), collector: collector);
        using var traces = BrokenTraceStore.Down();
        using var down = new StudioHost(StudioHost.Marketplace(), traces: traces);
    ...
    {
        using var studio = new StudioHost(StudioHost.Marketplace(), collectorAddress: "   ");
