...
public sealed partial class ArchitectureCheckTests
{
    ...
    {
        using var tree = new RulesTree()
            .Write("sketches/clock/ClockSketch.cs", "// The Widget ticks.\npublic sealed class ClockSketch;\n")
            .Write("sketches/clock/clockSketch.ts", "export const widget = 1;\n");
    ...
    {
        using var tree = new RulesTree()
            .Write("sketches/clock/utils/ClockSketch.cs", "// The Widget ticks.\npublic sealed class ClockSketch;\n");

        Assert.Equal([("banned-folder-names", "sketches/clock/utils")], tree.Breaches());
    }
    ...
    public void The_skipped_folders_come_from_the_words_file()
    {
        using var tree = new RulesTree()
            ...
            .Write("drafts/Clock.cs", "// The Widget ticks.\npublic sealed class Clock;\n")
            .Write("sketches/Clock.cs", "// The Widget ticks.\npublic sealed class Clock;\n");

        Assert.Equal([("banned-words", "sketches/Clock.cs")], tree.Breaches());
    }
    ...
    {
        using var tree = new RulesTree()
            .Set(WordsFile, "banned-words", "[Sprocket]")
